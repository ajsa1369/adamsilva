 ...
 *
 * Package definitions:
 *   genesis:    baseSetup=16000, baseMonthly=3500, t1Slots=3, t2Slots=0, t3Slots=0
 *   essentials: baseSetup=28000, baseMonthly=6500, t1Slots=6, t2Slots=1, t3Slots=0
 *   prime:      baseSetup=48000, baseMonthly=12000, t1Slots=12, t2Slots=3, t3Slots=1
 *   scale:      baseSetup=75000, baseMonthly=0, t1Slots=99, t2Slots=99, t3Slots=99
 *
...
describe('calculatePricing', () => {
  // Case 1: Slot logic — all slots used exactly (Genesis + 3 T1 → 0 overages)
  it('Genesis + 3 T1 integrations fills all T1 slots with zero overages', () => {
    const result = calculatePricing('genesis', [t1('Stripe'), t1('HubSpot'), t1('Calendly')])
    expect(result.packageSlug).toBe('genesis')
  ...
  // Case 2: Overage T1 — Genesis + 4 T1 → 1 overage
  it('Genesis + 4 T1 integrations produces 1 T1 overage ($750 setup, $150/mo)', () => {
    const result = calculatePricing('genesis', [
      t1('Stripe'),
      t1('HubSpot'),
      t1('Calendly'),
      t1('Mailchimp'),
  ...
  // Case 3: Overage T2 — Essentials + 2 T2 integrations → 1 overage (Essentials has 1 T2 slot)
  it('Essentials + 2 T2 integrations produces 1 T2 overage ($1500 setup, $250/mo)', () => {
    const result = calculatePricing('essentials', [t2('Brevo'), t2('Drip')])
  ...
  // Case 4: Mixed tier — Essentials + 3 T1 + 2 T2 → T2 overage only
  it('Essentials + 3 T1 + 2 T2: T1 fits in 6 slots, T2 slot=1 so 2nd T2 is overage', () => {
    const result = calculatePricing('essentials', [
      t1('Stripe'),
      t1('HubSpot'),
      t1('Calendly'),
      t2('Brevo'),
  ...
  // Case 5: Scale unlimited — Scale + 15 T1 → no overages
  it('Scale + 15 T1 integrations produces zero overages (99 unlimited slots)', () => {
    const integrations = Array.from({ length: 15 }, (_, i) => t1(`Tool${i + 1}`))
    const result = calculatePricing('scale', integrations)
    expect(result.overageIntegrations).toHaveLength(0)
  ...
  // Case 6: Empty integrations — Genesis + [] → setupTotal=16000
  it('Genesis with empty integration list returns base costs with no overages', () => {
    const result = calculatePricing('genesis', [])
    expect(result.setupTotal).toBe(16000)
    expect(result.monthlyTotal).toBe(3500)
    expect(result.overageIntegrations).toHaveLength(0)
  ...
  // Bonus: Prime T3 slot — Prime + 1 T3 included, 2nd T3 is overage
  it('Prime + 2 T3 integrations: 1st T3 included, 2nd T3 is overage ($4000 setup, $500/mo)', () => {
    const result = calculatePricing('prime', [t3('Mindbody'), t3('Vagaro')])
    expect(result.includedIntegrations).toHaveLength(1)
    expect(result.overageIntegrations).toHaveLength(1)
  ...
  // Bonus: Essentials + 6 T1 → all 6 T1 slots filled, no overages
  it('Essentials + 6 T1 integrations fills all T1 slots exactly with zero overages', () => {
    const integrations = [
      t1('Stripe'), t1('HubSpot'), t1('Calendly'),
      t1('Mailchimp'), t1('Pipedrive'), t1('Xero'),
    ]
    const result = calculatePricing('essentials', integrations)
    expect(result.overageIntegrations).toHaveLength(0)
